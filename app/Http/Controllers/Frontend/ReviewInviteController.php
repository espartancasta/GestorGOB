...
class ReviewInviteController extends Controller
{
    public function show($token)
    {
        $invite = SubmissionReviewer::where('invite_token_hash', $token)
            ->with(['submission.author', 'submission.files'])
            ->first();
        ...
        if ($invite->status === 'invited' && $invite->isExpired()) {
            ...
            return view('frontend.review-invite.show', [
                'invite' => $invite->fresh(['submission.author', 'submission.files']),
                'expired' => true,
            ]);
        }
    ...
    public function uploadReview(Request $request, $token)
    {
        ...
        $request->validate([
            'review_decision' => ['required', 'string', 'in:en_revision,aprobado,aprobado_observaciones,no_aprobado'],
            'file' => ['required', 'file', 'mimes:docx', 'max:10240'],
        ], [
            'review_decision.required' => 'Debes seleccionar el dictamen de tu revision.',
            'review_decision.in' => 'El dictamen seleccionado no es valido.',
        ...
        try {
            ...
            if ($completedReviews >= 2) {
                $submission = Submission::with('reviewers')
                    ->findOrFail($invite->submission_id);

                $decision = $submission->evaluationDecisionKey();

                $submission->update([
                    'estado' => $decision,
                    'status' => match ($decision) {
                        'aprobado_observaciones' => 'pending_correction',
                        'aprobado', 'no_aprobado' => 'completed',
                        default => 'in_review',
                    },
                ]);

                $submission->author?->notify(
                    new AuthorReviewReadyNotification($submission)
                );
            }
