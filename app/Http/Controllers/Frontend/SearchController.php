    ...
    public function index(Request $request)
    {
        if (Auth::check() && Auth::user()->role == 1) {
            $submissions = Submission::query()
                ->with(['author', 'reviewers.reviewer', 'files'])
                ->where('author_id', Auth::id())
                ->whereIn('status', ['waiting_acceptance', 'in_review', 'pending_correction', 'final_review', 'completed'])
                ->latest()
                ->get();
